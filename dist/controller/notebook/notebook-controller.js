...
class NotebookController extends entity_controller_1.EntityController {
    ...
    async showSingleEntityDetailsPage(entityID) {
        ...
        if (body.includes("{{MACRO_LIST_NOTES}}")) {
            body = body.replace("{{MACRO_LIST_NOTES}}", await this.showNotesInNotebook(this.authorizedUserName, entityID));
        }
        if (body.includes("{{MACRO_LIST_LINKS_TO_ADD_NOTES}}")) {
            body = body.replace("{{MACRO_LIST_LINKS_TO_ADD_NOTES}}", await this.showLinksToAddNotes(entityID));
        }
        const httpResponse = {
            ...response,
            body,
